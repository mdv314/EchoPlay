...
# from handler import process_queue
# from writer import write_to_queue

# def load_profiles():
#     profiles_dir = "profiles"  # Directory containing profile files
#     if not os.path.exists(profiles_dir):
#         os.makedirs(profiles_dir)  # Ensure the directory exists
#     profiles = [os.path.splitext(f)[0] for f in os.listdir(profiles_dir) if os.path.isfile(os.path.join(profiles_dir, f))]
#     profiles.append("+ edit profiles")
#     return profiles
...
# # function runs after clicking on "play"
# def play_action():
...
#         # Construct the path to the virtual environment's Python binary
#         venv_python = os.path.join(solution_dir, "echoplay", "bin", "python")

#         # multiprocessing call
#         start_handler(selected_profile)

#         # Run handler.py inside the virtual environment
#         # subprocess.run([venv_python, "handler.py", selected_profile])
...
#         queue.get_nowait()  # Remove item without blocking
...
# profile_menu = ttk.Combobox(tk_root, textvariable=profile_var, values=load_profiles(), state="readonly")
...
# refresh_button = tk.Button(tk_root, text="Stop", command=stop)
