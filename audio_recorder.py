...
class AudioToTextRecorder:
    ...
    def __init__(self,
        ...
        self.desired_overlap_duration = desired_overlap_duration
    ...
    def _realtime_worker(self):
        ...
        This simplified version only calls the realtime update callback.
        """
        try:
            ...
            while self.is_running:
                if self.is_recording:
                    ...
                    # Transcription branch depending on model usage
                    if self.use_main_model_for_realtime:
                        ...
                        # Perform transcription using the alternative branch
                        if self.realtime_batch_size > 0:
                    ...
                    # Check if still recording and that enough time has passed since starting
                    if self.is_recording and time.time() - self.recording_start_time > self.init_realtime_after_seconds:
                        # Simplify by just stripping the text and calling the realtime update callback
                        with self.transcription_lock:
                            ...
                            num_frames_to_keep = int(self.desired_overlap_duration * self.sample_rate)
                            self.frames = self.frames[-num_frames_to_keep:]
                            self.realtime_transcription_text = ''
                        # if len(self.realtime_transcription_text) > 10:
                        #     self.realtime_transcription_text = ''
                        #     print("Cleared realtime_transcription_text")
            ...
            logging.error(f"Unhandled exception in _realtime_worker: {e}", exc_info=True)
            raise
